var API_ENDPOINT = "https://bitsgap.com/api2";
...
NioApp = function (NioApp, $, window, document) {
    ...
    NioApp.MainMenu = function () {
        ...
        MenuInit.navClose = function () {
            ...
            $doc.on('click', 'body', function (e) {
                // disabled auto hiding of menu if auto click on slider:
                var isNotSlider = e.target.className !== 'bx-next';
                //
                if (!$menu_toggle.is(e.target) && $menu_toggle.has(e.target).length === 0 && !$header.is(e.target) && $header.has(e.target).length === 0 && $win.width() < _navBreak && isNotSlider) {
                    $navbar_toggle.removeClass('active');
    ...
    //scrollAct @v1.0
    var bootstrap =  addScript("/assets/js/vendors/bootstrap.min.js");
    ...
    var countdown = addScript("/assets/js/vendors/jquery.countdown.min.js");
    ...
    // Popup !Plugin @v1.0
        NioApp.Plugins.popup = function () {
            ...
            $("#modal-video").on('shown.bs.modal', function (e) {
                const videoID = $(e.relatedTarget).data('vid');
                popupInit.video_popup();
                $('#modal-video .modal-youtube').append(`
                    <iframe width="100%" height="100%" src="https://www.youtube.com/embed/${videoID}?autoplay=1&rel=0&vq=hd1080"
    ...
    NioApp.Util.wordSplit = function () {
        ...
        if ($word_mask.exists()) {
            $word_mask.each(function () {
                var $self = $(this), _words = $self.text().split(' '), join_words = '',
                    wd_dly = ($self.data('delay') > 0) ? $self.data('delay') : 1;
        ...
        if ($elem_mask.exists()) {
            $elem_mask.each(function () {
                var $self = $(this), _element = $self.html(),
                    el_dly = ($self.data('delay') > 0) ? $self.data('delay') : 1;
